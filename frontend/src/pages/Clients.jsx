...
export default function Clients() {
  ...
  const fetchWithTimeout = (url, options, timeout = 80000) =>
    Promise.race([
      fetch(url, options),
    ...
    const fetchData = async () => {
      ...
      const headers = { Authorization: `Bearer ${activeToken}` };
        ...
        if (pag_info.status === "fulfilled") {
          ...
          const pages = Number.isInteger(totalPagesRaw)
            ? totalPagesRaw
  ...
  return (
    <div id="clients" className="min-h-screen flex relative flex-col ">
      ...
      <div className="z-20 px-5 -mt-10 flex justify-end items-center ">
        ...
        <Link
          to="Adduser"
          className="flex justify-between px-3 py-2 bg-white w-30 rounded-lg hover:font-black shadow-sm font-light"
        >
          ...
          ? 
            ...
            filteredUsers.map((user, index) => (
              <div
                ...
                key={index}
              >
                <h1 className="text-xl font-medium pl-5 text-gray-800">
                  {user.name}
                </h1>
                <h1 className="text-xl font-medium text-gray-800">
                  {user.email}
                </h1>
                <h1 className="text-xl font-medium text-gray-800 pl-50">
                  {user.role}
                </h1>
